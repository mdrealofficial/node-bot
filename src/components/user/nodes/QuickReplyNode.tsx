...
export const QuickReplyNode = memo(({ data }: NodeProps) => {
  const status = getNodeStatus("quickReply", data);
  ...
  return (
    <div className={`px-4 py-3 shadow-lg rounded-lg bg-card border-2 min-w-[200px] min-h-[100px] cursor-pointer hover:shadow-xl transition-shadow relative ${
      status.isValid ? 'border-blue-500' : 'border-yellow-500'
    }`}>
      ...
      <div className="text-xs font-medium text-foreground mb-1">
        {String(data.replyText || "Unnamed Quick Reply")}
      </div>
      <div className="text-[10px] text-muted-foreground">
        {actionType === 'start_flow' ? '🔄 Start Flow' : '➡️ Next Message'}
      </div>
