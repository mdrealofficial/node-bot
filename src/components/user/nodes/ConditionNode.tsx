import { memo } from "react";
import { Handle, Position, NodeProps } from "@xyflow/react";
import { GitBranch, CheckCircle, AlertCircle } from "lucide-react";
import { getNodeStatus } from "./nodeValidation";
import { ConnectionBadge } from "./ConnectionBadge";

export const ConditionNode = memo(({ data }: NodeProps) => {
  const hasCondition = data.condition && 
    typeof data.condition === 'object' && 
    'field' in data.condition;
  const status = getNodeStatus("condition", data);

  return (
    <div className={`px-4 py-2 shadow-lg rounded-lg bg-card border-2 min-w-[200px] cursor-pointer hover:shadow-xl transition-shadow relative ${
      status.isValid ? 'border-orange-500' : 'border-yellow-500'
    }`}>
      <Handle type="target" position={Position.Left} className="w-3 h-3" />
      
      {/* Connection Count Badge */}
      <ConnectionBadge 
        current={(data.connectionCount as number) || 0} 
        max={(data.maxConnections as number) || 2} 
      />
      
      {/* Validation Badge */}
      {!status.isValid && (
        <div className="absolute -top-2 -right-2 w-6 h-6 bg-yellow-500 rounded-full flex items-center justify-center animate-pulse shadow-lg">
          <AlertCircle className="h-4 w-4 text-white" />
        </div>
      )}
      
      <div className="flex items-center justify-center mb-2">
        <GitBranch className="h-4 w-4 text-orange-500" />
      </div>
      
      {status.isValid && hasCondition ? (
        <>
          <div className="text-xs text-muted-foreground mb-1">
            If <span className="font-semibold">{String((data.condition as any).field)}</span>
          </div>
          <div className="text-xs text-muted-foreground">
            {String((data.condition as any).operator).replace(/_/g, ' ')} "{String((data.condition as any).value)}"
          </div>
        </>
      ) : (
        <div className="text-xs text-yellow-600 dark:text-yellow-400 font-medium">
          {status.message}
        </div>
      )}
      
      <div className="flex justify-between text-xs text-muted-foreground mt-2">
        <span>✓ True</span>
        <span>✗ False</span>
      </div>
      
      <Handle type="source" position={Position.Right} id="true" className="w-3 h-3 top-[30%]" />
      <Handle type="source" position={Position.Right} id="false" className="w-3 h-3 top-[70%]" />
    </div>
  );
});

ConditionNode.displayName = "ConditionNode";
